...
/**
 * Filename: ChineseSpelling.java Description: Copyright: Copyright (c)2009
 * Company: company
 * 
 * @author: liangbo
 * @version: 1.0 Create at: 2010-9-5 下午09:52:37
 * 
 *           Modification History: Date Author Version Description
 *           ------------------------------------------------------------------
 *           2010-9-5 liangbo 1.0 1.0 Version
 */
...
//		tNameSpell = finder.getSelling(displayName);
    	tNameSpell = ChineseSpelling.Cn2Spell(displayName);
    	
    	 // english is always before of chinese
//        if (isChinese) {
//        	sortKeyPrimary = sortKeyPrimary.substring(0, 1) + "奡" + sortKeyPrimary; 
//        	sortKeyAlternative = sortKeyAlternative.substring(0, 1) + "z" + sortKeyAlternative; 
//        } else {
//        	sortKeyPrimary = sortKeyPrimary.substring(0, 1) + " " + sortKeyPrimary; 
//        }
    	
		if(displayName!=null && (!displayName.equals(tNameSpell)))
		{
			//中文
			tNameSpell =/*"�S"+*/tNameSpell;
			tNameSpell = tNameSpell.substring(0, 1) + "奡奡" + tNameSpell; 
		}
		else
		{
			//String fStr = displayName.substring(0, 1);
			char fchar= displayName.charAt(0);
			//判断是否字符大写  如果大写 转小写
			if(Character.isUpperCase(fchar))
			{
				tNameSpell = Character.toLowerCase(fchar)  + "奡 " + displayName;
			}
			else
			{
				tNameSpell = displayName.substring(0, 1) + " " + displayName; 
	...
	public static String Cn2Spell(String str){
		...
		for(int i=0;i<num;i++){
			char ch = name[i];
			String wordSpell[] = PinyinHelper.toHanyuPinyinStringArray(ch);
		
			/*for(int j=0;j<wordSpell.length;j++)
			{ 
				if(wordSpell[j].length()>0){
					wordSpell[j]= wordSpell[j].substring(0,(wordSpell[j].length())-1);
					}

				if(j==0||(j>0&&!wordSpell[j].equals(wordSpell[j-1]))){
					bb.append(wordSpell[j]);
				}
			}*/
			if (wordSpell==null){		
				bb.append(String.valueOf(ch));		
			}else if(wordSpell!=null&&wordSpell.length>0){
				wordSpell[0]= wordSpell[0].substring(0,(wordSpell[0].length())-1);
				bb.append(wordSpell[0]);
			}
		}
		return bb.toString();
	}	
	...
	static final boolean  IS_UTF_8 =true; 
