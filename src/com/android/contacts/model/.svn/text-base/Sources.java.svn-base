 ...
 * 数据源头，同时是  广播监听器，接收处理广播，当账户修改的时候
 */
public class Sources extends BroadcastReceiver implements OnAccountsUpdateListener {
     ...
     *真实的在账户变化后的处理函数  ，可能是广播引起 也可能是账户管理器监听引起
     */
        ...
        try {
            ...
            for (SyncAdapterType sync : syncs) {
                if (!ContactsContract.AUTHORITY.equals(sync.authority)) {
                	...
                	//跳过这个同步是配置器，这个不是联系人的同步器 
                    continue;
                }

                // Look for the formatting details provided by each sync
                // adapter, using the authenticator to find general resources.
                //寻找每个同步适配器提供的格式详细信息，使用验证，以找出一般资源。
                final String accountType = sync.accountType;//从同步器中得到账号唯一标识
                //利用同步器中的账号标识，从账号管理器中得到账号的信息
     ...
     * 利用 唯一标识的字符串验证器accountType 从全部的账号信息数组中，取得需要的账号信息
     */
