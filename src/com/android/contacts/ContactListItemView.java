...
public class ContactListItemView extends ViewGroup {
    ...
    private final int mPhoneTextMargin;
    ...
    private boolean mVerticalDividerVisible1;
    private boolean mVerticalDividerVisible2;
    private Drawable mVerticalDividerDrawable1;
    private Drawable mVerticalDividerDrawable2;
    ...
    public ContactListItemView(Context context, AttributeSet attrs) {
        ...
        mPhoneTextMargin=
        		resources.getDimensionPixelOffset(R.dimen.list_item_phone_text_margin);
        mNameTextWidth=
        		resources.getDimensionPixelOffset(R.dimen.list_item_name_text_width);
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if(isVisible(mPhoneTextView)){
        	mPhoneTextView.measure(0,0);
        }
        
        if(isVisible(mMsgButton)){
        	mMsgButton.measure(0, 0);
        }
        ensurePhotoViewSize();
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (isVisible(mMsgButton)) {
            int buttonWidth = mMsgButton.getMeasuredWidth();
            ...
            mMsgButton.layout(
                    ...
                    rightBound + buttonWidth,
                    height);
            mVerticalDividerVisible1 = true;
            ensureVerticalDivider1();
            rightBound -= mVerticalDividerWidth;
            mVerticalDividerDrawable1.setBounds(
                    rightBound,
                    topBound + mVerticalDividerMargin,
                    rightBound + mVerticalDividerWidth,
                    height - mVerticalDividerMargin);
        } else {
            mVerticalDividerVisible1 = false;
        }
        ...
        if (isVisible(mCallButton)) {
            ...
            mCallButton.layout(
                    ...
                    rightBound + buttonWidth,
                    height);
            mVerticalDividerVisible2 = true;
            ensureVerticalDivider2();
            rightBound -= mVerticalDividerWidth;
            mVerticalDividerDrawable2.setBounds(
                    rightBound,
                    topBound + mVerticalDividerMargin,
                    rightBound + mVerticalDividerWidth,
                    height - mVerticalDividerMargin);
        } else {
            mVerticalDividerVisible2 = false;
        }
...
//                textTopBound + mLine1Height);
    	...
    	/* Add begin, custom by 139's contacts */
        if(isVisible(mPhoneTextView)){
        	mPhoneTextView.layout(leftBound+mPhoneTextMargin+mNameTextWidth, 
        			textTopBound, 
        			leftBound+mPhoneTextMargin+(2*mNameTextWidth), 
        			textTopBound + mLine1Height);
        }
    	/* Add end, custom by 139's contacts */
    }
     ...
     */
    private void ensureVerticalDivider1() {
        if (mVerticalDividerDrawable1 == null) {
            mVerticalDividerDrawable1 = mContext.getResources().getDrawable(
            		com.android.internal.R.drawable.divider_vertical_dark_opaque);
            //mVerticalDividerWidth = mVerticalDividerDrawable1.getIntrinsicWidth();
            mVerticalDividerWidth = 1;
        }
    }
    
    private void ensureVerticalDivider2() {
        if (mVerticalDividerDrawable2 == null) {
            mVerticalDividerDrawable2 = mContext.getResources().getDrawable(
            		com.android.internal.R.drawable.divider_vertical_dark_opaque);
           // mVerticalDividerWidth = mVerticalDividerDrawable2.getIntrinsicWidth();
            mVerticalDividerWidth = 1;
        }
    }
    /**
    ...
    public void dispatchDraw(Canvas canvas) {
        ...
        if (mVerticalDividerVisible1) {
            mVerticalDividerDrawable1.draw(canvas);
        }
        if (mVerticalDividerVisible2) {
            mVerticalDividerDrawable2.draw(canvas);
        }
    ...
    public TextView getPhoneTextView(){
        if (mPhoneTextView == null) {
        	mPhoneTextView = new TextView(mContext);
        	mPhoneTextView.setSingleLine(true);
        	mPhoneTextView.setEllipsize(TruncateAt.MARQUEE);
        	mPhoneTextView.setTextAppearance(mContext, android.R.style.TextAppearance_Large);
        	mPhoneTextView.setGravity(Gravity.CENTER_VERTICAL);
            addView(mPhoneTextView);
        }
    ...
    public void showCallButton(int id, int tag) {
        if (mCallButton == null) {
        	...
        	mCallButton = new ImageView(mContext, null);
            ...
            mCallButton.setImageResource(R.drawable.icon_call);
    ...
    public void showMsgButton(int id,int tag){
        if (mMsgButton == null) {
        	//mMsgButton = new DontPressWithParentImageView(mContext, null);
        	mMsgButton = new ImageView(mContext, null);
        	mMsgButton.setId(id);
        	mMsgButton.setOnClickListener(mCallButtonClickListener);	
        	mMsgButton.setBackgroundResource(R.drawable.call_background);
        	mMsgButton.setImageResource(R.drawable.icon_msg);
        	mMsgButton.setPadding(mCallButtonPadding, 0, mCallButtonPadding, 0);
        	mMsgButton.setScaleType(ScaleType.CENTER);
            addView(mMsgButton);
        }

        mMsgButton.setTag(tag);
        mMsgButton.setVisibility(View.VISIBLE);
    	
    }

    public void hideMsgButton() {
        if (mMsgButton != null) {
        	mMsgButton.setVisibility(View.GONE);
        }
    }
