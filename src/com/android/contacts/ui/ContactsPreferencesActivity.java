...
public final class ContactsPreferencesActivity extends ExpandableListActivity implements
    ...
    private SharedPreferences filter = null;//移植过来的，用于代替mPrefs进行过滤
    private ContactsPreferences mContactsPrefs;

    private CheckBox mDisplayPhones;
    private CheckBox mDisplaySim;
    private CheckBox mDisplayNet;

    private View mHeaderPhones;
    private View mHeaderSeparator;
    private View mGroupSim;
    private View mGroupNet;
    ...
    protected void onCreate(Bundle icicle) {
        ...
        createDisplayGroupSim(inflater);
        createDisplayGroupNet(inflater);
    ...
    private void createWithPhonesOnlyPreferenceView(LayoutInflater inflater) {
        ...
        mDisplayPhones.setChecked(filter.getBoolean(Constants.FILTER_SEE_ONLY_PHONE, false));
        {
    ...
    private void createDisplayGroupSim(LayoutInflater inflater) {
    	// Add sim 
        mGroupSim = inflater.inflate(R.layout.display_options_sim_only, mList, false);
       // mGroupSim.setId(R.id.group_sim);
        mDisplaySim = (CheckBox) mGroupSim.findViewById(android.R.id.checkbox);
        mDisplaySim.setChecked(filter.getBoolean(Constants.FILTER_SEE_ONLY_SIM, true));
        {
            final TextView text_sim_1 = (TextView)mGroupSim.findViewById(android.R.id.text1);
            final TextView text_sim_2 = (TextView)mGroupSim.findViewById(android.R.id.text2);
            text_sim_1.setText(R.string.preference_see_only_sim);
            text_sim_2.setText(R.string.see_only_sim);
        }
    }
    
    private void createDisplayGroupNet(LayoutInflater inflater) {
    	// Add Net 
        mGroupNet = inflater.inflate(R.layout.display_options_net_only, mList, false);
       // mGroupSim.setId(R.id.group_sim);
        mDisplayNet = (CheckBox) mGroupNet.findViewById(android.R.id.checkbox);
        mDisplayNet.setChecked(filter.getBoolean(Constants.FILTER_SEE_ONLY_NET, true));
        {
            final TextView text_net_1 = (TextView)mGroupNet.findViewById(android.R.id.text1);
            final TextView text_net_2 = (TextView)mGroupNet.findViewById(android.R.id.text2);
            text_net_1.setText(R.string.preference_see_only_net);
            /***************设置登陆账号提示*************/
            try {
    		...
    		String pre_account_name=null;
    		String end_account_name=null;
    		if (account_name!=null&&account_name.length()>0){
    			pre_account_name = account_name.substring(0,3);
    			end_account_name = account_name.substring(account_name.length()-4,account_name.length());
    			text_net_2.setText("显示业务通行证号码("+pre_account_name+"****"+end_account_name+")的网络联系人");
    		}else{
    			text_net_2.setText("未检测到登陆用户");
    		}
    	}
    }
    @Override
    protected void onResume() {
        ...
        mList.removeHeaderView(mGroupSim);
        mList.removeHeaderView(mGroupNet);
        ...
        mList.addHeaderView(mGroupSim, null, true);
        mList.addHeaderView(mGroupNet, null, true);
	...
	public void onClick(View v) {
		...
		case R.id.btn_done:
			...
			if (mDisplayPhones != null) {
				editor.putBoolean(Constants.FILTER_SEE_ONLY_PHONE, mDisplayPhones
						.isChecked());
			}
			if (mDisplaySim != null) {
				editor.putBoolean(Constants.FILTER_SEE_ONLY_SIM, mDisplaySim
						.isChecked());
			}
			if (mDisplayNet != null) {
				editor.putBoolean(Constants.FILTER_SEE_ONLY_NET, mDisplayNet
						.isChecked());
			}
			editor.commit();
			setResult(RESULT_OK);
