...
public class ContactsUtils {
	...
	public static String formatDuration(Context context, long elapsedSeconds) {
		long minutes = 0;
		long seconds = 0;
		...
		return context.getString(R.string.callDetailsDurationFormat, minutes,
				seconds);
	}

	/**
	 * Initialize to search view. The targe Activity must have a EditText
	 * whose id attribute is "to_search_src_text" on content view.
	 * @param target	must be not null
	 */
	public static void initToSearchView(final Activity target) {/*
		/*if (target == null) {
			Log.d(TAG, "The target activity is null");
			return;
		}
		EditText et = (EditText)target.findViewById(R.id.to_search_src_text);
		if (et == null) {
			Log.d(TAG, "Your content must have a EditText whose id attribute is "
					+ "'to_search_src_text'");
			return;
		}
		et.setImeOptions(EditorInfo.IME_NULL);		//no keyboard
		et.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				target.onSearchRequested();
			}
		});
	...
	public static void initBottomBtns(final Activity target) {
		/*View v = target.findViewById(R.id.ll_btn_show_allcall);
		if (v != null) {
		...
		/*v = target.findViewById(R.id.ll_btn_show_book);
		if (v != null) {
			v.setOnClickListener(new OnClickListener() {
				public void onClick(View v) {
					Intent intent = new Intent(target,
	...
	/**
	 * 根据联系人账号类型判断是否为网络联系人。除了SIM卡上的联系人之外，其它的都为网络联系人。
	 * @param contactType 联系人账号类型 
	 * @see RawContacts#ACCOUNT_TYPE
	 * @return 
	 */
	public static boolean isWebContact(String contactType) {
		return Config.ACCOUNT_TYPE_139.equals(contactType);
	}
}
