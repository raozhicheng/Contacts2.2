...
public class ContactPhotoLoader implements Callback {
     ...
     * from the database.
     */
    public void loadPhoto(ImageView view, long photoId,boolean isWeb) {
        if (photoId == 0) {
        	if(isWeb){
        		view.setImageResource(R.drawable.ic_contact_web);
        	}else{
        		view.setImageResource(R.drawable.ic_contact_sim);        		
        	}
